 ...
 */
export function kindsFromTokens(selectorText) {
 ...
 */
export function kindsFromTrailingApi(trailing) {
    if (/^\s*\.\s*(fill|pressSequentially)\(/.test(trailing))
        return ['textbox'];
    if (/^\s*\.\s*(check|uncheck)\(/.test(trailing))
        return ['checkbox', 'radio'];
    if (/^\s*\.\s*selectOption\(/.test(trailing))
        return ['combobox'];
    return [];
}
/** The candidate element's actual kind; null when it has no clear kind. */
export function kindOfElement(info) {
    if (info.role) {
 ...
 */
export function kindConflict(expected, actual) {
