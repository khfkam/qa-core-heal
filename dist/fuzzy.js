 ...
 */
export function isGeneratedIdentifier(v) {
    if (/^[_:]?r[_:]?(?=[0-9a-z]*\d)[0-9a-z]+[_:.-]/i.test(v))
        return true;
    const words = v.split(/[^A-Za-z0-9]+/).filter(Boolean);
    if (words.length === 0)
        return true;
    const generated = (w) => /^\d+$/.test(w) || (/^[0-9a-f]{4,}$/i.test(w) && /\d/.test(w));
...
const GENERATED_WORD = (w) => /^\d+$/.test(w) || (/^[0-9a-f]{4,}$/i.test(w) && /\d/.test(w));
/**
 ...
 * in a refusal.
 */
export function isSelectorLikeText(v) {
    const t = v.trim();
 ...
 * "search-accounts" is.
 */
function containmentScore(sourceTokens, value, attr) {
    ...
    if (!found)
        return 0;
    if (attr !== undefined && NAME_ISH.has(attr))
 ...
 */
export function matchFuzzy(source, candidates) {
    ...
    const prefixScore = (v) => {
        if (sourceTokens.length < 2 || sourceTokens.join('').length < MIN_PREFIX_LENGTH)
            return 0;
        const vt = wordTokens(v);
        if (vt.length !== sourceTokens.length)
            return 0;
        for (let i = 0; i < sourceTokens.length - 1; i++) {
            if (vt[i] !== sourceTokens[i])
                return 0;
        }
    ...
    for (const c of candidates) {
        ...
        for (const v of c.values) {
            if (isSelectorLikeText(v))
                continue;
            if (isGeneratedIdentifier(v)) {
                const g = similarity(source, v);
                if (!bestGenerated || g > bestGenerated.score)
                    bestGenerated = { value: v, score: g };
                continue;
            }
            const s = Math.max(similarity(source, v), containmentScore(sourceTokens, v, c.attrOf?.[v]), prefixScore(v));
            if (!best || s > best.score)
                best = { value: v, score: s };
        }
        if (best)
            scored.push({ candidate: c, value: best.value, score: best.score });
        else if (bestGenerated)
            generatedOnly.push({ candidate: c, value: bestGenerated.value, score: bestGenerated.score });
    }
    ...
    if (band.length > 1) {
        ...
        if (strippedSource)
            exactForms.add(normalizeIdentifier(strippedSource));
    ...
    if (!shortToken) {
        const top = (list) => list.filter((s) => s.score >= FUZZY_NEAR_MISS_FLOOR)
            .sort((a, b) => b.score - a.score)
            .slice(0, 3)
            .map((s) => ({ display: s.candidate.display, score: s.score }));
        const closest = top(scored);
        if (closest.length > 0)
            return { kind: 'near-miss', closest };
        ...
        if (generatedClosest.length > 0)
            return { kind: 'near-miss', closest: generatedClosest };
    }
    return { kind: 'none' };
}
