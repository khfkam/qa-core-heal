 ...
 */
export function parseGotos(src) {
    const out = [];
    const re = /\.goto\(\s*/g;
    let m;
    while ((m = re.exec(src)) !== null) {
        ...
        if (prop)
            out.push({ line, route: prop[1] });
    }
    return out;
}
/** Route governing a locator at `line`: nearest goto above, else the first. */
export function routeForLine(gotos, line) {
    if (gotos.length === 0)
        return null;
    let best = null;
    for (const g of gotos) {
        if (g.line <= line && (!best || g.line > best.line))
            best = g;
    }
...
export function stripAutoSuffixes(token) {
    ...
    if (end === words.length) {
        ...
        if (m)
            return [...words.slice(0, -1), m[1]].join(' ');
        return null;
    }
    return words.slice(0, end).join(' ');
}
function pathSuffixMatches(filePath, pattern) {
...
export function buildRoutePlan(input) {
    ...
    for (const f of input.files) {
        ...
        if (o)
            overrideByFile.set(f.path, o.route);
    }
    const gotosByFile = new Map();
    for (const f of input.files)
        gotosByFile.set(f.path, parseGotos(f.src));
    ...
    const inheritedByFile = new Map();
    for (const f of input.files) {
        if (overrideByFile.has(f.path) || (gotosByFile.get(f.path)?.length ?? 0) > 0)
            continue;
        const routes = [];
        for (const [spec, filePaths] of input.specFiles) {
...
export function routesForLocator(plan, filePath, line) {
    const override = plan.overrideByFile.get(filePath);
    if (override)
        return [resolveRoute(plan.baseUrl, override)];
    const own = routeForLine(plan.gotosByFile.get(filePath) ?? [], line);
    if (own)
        return [resolveRoute(plan.baseUrl, own)];
 ...
 */
export function hasExplicitRoute(plan, filePath, line) {
    if (plan.overrideByFile.has(filePath))
        return true;
    if (routeForLine(plan.gotosByFile.get(filePath) ?? [], line) !== null)
        return true;
