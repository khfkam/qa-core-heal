 ...
 * action.
 */
export function classifyFailure(rawMessage) {
    ...
    if (/\bexpect(?:\(locator\))?\.(?:toHaveCount|toBeVisible|toBeAttached|toBeInViewport)\b/.test(msg)
 ...
 */
export function extractSelector(msg) {
...
export function parseConsent(answer) {
 ...
 */
export function traceFailureUrl(zipPath) {
    let entries;
    try {
    ...
    for (const name of names) {
        for (const line of entries.get(name).toString('utf8').split('\n')) {
            if (!line.includes('"frame-snapshot"'))
                continue;
            try {
                const o = JSON.parse(line);
                if (o.type !== 'frame-snapshot')
                    continue;
                if (o.snapshot?.isMainFrame === false)
                    continue;
        ...
        for (const spec of s.specs ?? []) {
            ...
            const pushLoc = (file, line) => {
                ...
                if (locations.length >= 6)
                    return;
                if (!locations.some((l) => l.file === file && l.line === line))
