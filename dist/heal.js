...
import { emitLocatorCall, resolve as resolveIntent } from './selectors.js';
...
import { buildRoutePlan, routesForLocator, routeLabel, resolveRoute, stripAutoSuffixes, hasExplicitRoute, } from './routes.js';
import { kindsFromTokens, kindsFromTrailingApi, kindOfElement, kindConflict } from './kind.js';
 ...
 */
export function applyHealPlan(plan) {
    const written = [];
 ...
 */
export function applyHealPlanExcluding(plan, excluded) {
    for (const entry of plan) {
        if (!entry.edits.some((e) => excluded(entry.file, e)))
            continue;
...
const ROLE_BOOL_OPTS = ['checked', 'disabled', 'expanded', 'includeHidden', 'pressed', 'selected'];
    ...
    switch (method) {
        ...
        case 'locator': {
            ...
            if (s.startsWith('xpath='))
                return { xpath: s.slice('xpath='.length), hasText };
            if (s.startsWith('//') || s.startsWith('./') || s.startsWith('(//'))
                return { xpath: s, hasText };
 ...
 */
export function parseLocatorCalls(src, file, wrappers = []) {
    const calls = [];
    ...
    const rootRe = new RegExp(`(?<![\\w.$])(this\\.page|page${wrapperRoots.length ? `|${wrapperRoots.join('|')}` : ''})(?![\\w$])`, 'g');
    let rm;
 ...
 */
function emitReplacement(call, level, arg, frameChain) {
    ...
    if (call.root === 'this.page' || call.wrapper)
        newRaw = 'this.' + newRaw;
    return newRaw;
}
/**
 ...
 */
export function collectSpecFiles(root, note) {
    const out = [];
    const walk = (dir) => {
        let entries;
        try {
            ...
            note?.(`[verbose] skipping unreadable directory ${dir}: ${e.message}`);
            return;
        }
        for (const entry of entries.sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            if (/\.(spec|test)\.(ts|js)$/.test(entry.name))
                ...
                note?.(`[verbose] skipping non-spec file ${p}`);
        }
    };
    walk(root);
    return out.sort();
 ...
 */
function gatherFiles(specPath, specSrc, followImports, pageObjectDirs, onFileError) {
 ...
 */
async function resolveBaseUrl(files, roots, baseUrl, project) {
    let configStorageState = null;
    let url = baseUrl ?? null;
    for (const root of roots) {
        if (url && configStorageState)
            break;
        const res = await resolvePlaywrightConfig(root, project);
        if (!res)
            continue; // no config in this root
        if (!configStorageState && res.storageState)
            configStorageState = res.storageState;
        if (url)
            continue;
        if (res.baseUrl) {
            url = res.baseUrl;
 ...
 */
async function probeIdentity(handle) {
    return handle.evaluate(async (el, windowMs) => {
        ...
        const read = () => {
            ...
            for (let i = 0; i < ATTRS.length; i++) {
                ...
                if (v)
                    out.push(['attr:' + ATTRS[i], v]);
            }
            const inp = el;
            if (typeof inp.value === 'string' && inp.value)
                out.push(['prop:value', inp.value]);
            ...
            for (const r of records) {
                if (r.type === 'attributes' && r.attributeName) {
                    ...
                    if (r.attributeName === 'id')
                        key = 'id';
                    else if (ATTRS.indexOf(r.attributeName) >= 0)
                        key = 'attr:' + r.attributeName;
                    if (key) {
                        mutated.add(key);
                        if (r.oldValue)
                            observed.push([key, r.oldValue]);
                    }
                }
                else {
                    mutated.add('text');
                    if (r.type === 'characterData' && r.oldValue)
                        observed.push(['text', r.oldValue]);
                }
            }
        });
        observer.observe(el, {
...
async function readFingerprint(locator) {
    for (let i = 0; i < 3; i++) {
        try {
            return await locator.first().evaluate((el) => {
                if (!el.isConnected)
                    throw new Error('detached mid-render');
...
function fingerprintsAgree(a, b) {
    ...
    if (!a.box || !b.box)
        return true;
    const close = (x, y) => Math.abs(x - y) <= 5;
...
function describeFingerprint(f) {
    ...
    if (f.type)
        out += `[type="${f.type}"]`;
    if (!f.id && !f.name && f.text)
        out += ` "${f.text}"`;
    if (f.box)
        out += ` at (${f.box.x}, ${f.box.y})`;
    return out;
}
/**
...
async function confirmSameElement(locator, token) {
    const nt = norm(token);
    if (nt.length < 2)
        return { confirmed: false, unstableMatch: false }; // nothing specific enough to confirm against
    ...
    const matches = (s) => {
        const ns = norm(s);
        if (ns && (ns.includes(nt) || (ns.length >= 3 && nt.includes(ns))))
            return true;
        const ss = normSep(s);
        if (ss && (ss.includes(st) || (ss.length >= 3 && st.includes(ss))))
            return true;
    ...
    const attempt = async () => {
        ...
        for (const [, oldValue] of probe.observed)
            discarded.push(oldValue);
        if (stable.some(matches))
            return 'confirmed';
        if (discarded.some(matches))
            return 'unstable';
        return 'nomatch';
    };
    ...
    if (verdict === 'confirmed')
        return { confirmed: true, unstableMatch: false };
    if (verdict === 'unstable')
        return { confirmed: false, unstableMatch: true };
 ...
 * same identity.
 */
function argsSignature(method, args) {
    const sig = { method };
    if (method === 'getByRole') {
        sig.role = args.role ?? '';
        if (args.name)
        ...
        for (const k of ROLE_BOOL_OPTS) {
            if (args.roleOpts?.[k] !== undefined)
                sig[k] = args.roleOpts[k];
        }
        if (args.roleOpts?.level !== undefined)
            sig.level = args.roleOpts.level;
    }
    else if (method === 'locator') {
        if (args.xpath)
            sig.xpath = args.xpath;
        else
            sig.css = args.css ?? '';
        if (args.hasText)
            sig.hasText = args.hasText;
    }
    else {
...
export function selectorSignature(text) {
    const t = text.trim();
...
function stateDependencyHint(call) {
    const a = call.args;
    ...
    for (const s of sources) {
        ...
        for (const w of words) {
            if (STATE_TOKENS.has(w.toLowerCase()))
...
function withCompoundHint(call, reason) {
    if (!isCompoundCss(call))
        return reason;
 ...
 */
async function scanIdentifiers(page, settleCapMs) {
    return page.evaluate(async (capMs) => {
        ...
        for (const root of roots) {
            ...
            for (const l of Array.from(root.querySelectorAll('label[for]'))) {
                ...
                if (t && f && !labelFor.has(f))
                    labelFor.set(f, t);
            }
            const attrEls = Array.from(root.querySelectorAll('[id], [name], [aria-label], [data-testid], [data-test], [placeholder]')).slice(0, 2000);
            for (const el of attrEls) {
                ...
                if (el.id && labelFor.has(el.id))
                    add(el, labelFor.get(el.id), 'label');
            }
            ...
            const textEls = Array.from(root.querySelectorAll(root === document ? 'body *' : '*')).slice(0, 4000);
            for (const el of textEls) {
                ...
                if (tag === 'label' || tag === 'option' || tag === 'script' || tag === 'style' || tag === 'noscript'
                    ...
                    continue;
                if (el.children.length > 0 || el.closest('label'))
                    continue;
                const t = (el.textContent ?? '').replace(/\s+/g, ' ').trim();
                if (!t || t.length > 80)
                    continue;
        ...
        const generatedId = (v) => /^[_:]?r[_:]?(?=[0-9a-z]*\d)[0-9a-z]+[_:.-]/i.test(v)
 ...
 */
export function tagTypoCorrections(css) {
    ...
    if (tag.includes('_')) {
        ...
        for (const known of KNOWN_TAGS) {
            if (known === stripped || oneEditApart(stripped, known))
                out.push(known + rest);
        }
        return out.sort();
    }
 ...
 */
export function closestKnownTag(token, maxDist) {
    let best = null;
    let bestD = maxDist + 1;
    for (const known of KNOWN_TAGS) {
...
export async function heal(opts) {
    ...
    if (givenPaths.length === 0)
        throw new Error('No spec path given.');
    const write = opts.write !== false;
    const verboseNote = opts.verbose ? (line) => console.error(line) : undefined;
    ...
    for (const sp of givenPaths) {
        if (!fs.existsSync(sp))
            throw new Error(`Spec not found: ${sp}`);
        if (fs.statSync(sp).isDirectory()) {
            const found = collectSpecFiles(sp, verboseNote);
            if (found.length === 0)
                throw new Error(`No spec files (*.spec.ts/js, *.test.ts/js) found under directory: ${sp}`);
    ...
    const fileErrors = [];
    const relSpec = (p) => path.relative(process.cwd(), p).split(path.sep).join('/');
    const fileError = (operation, p, spec, e) => {
    ...
    for (const sp of specPaths) {
        ...
        try {
            ...
            if (!st.isFile())
                throw new Error(`EISDIR: not a regular file`);
        ...
        const gathered = gatherFiles(sp, src, opts.followImports !== false, opts.pageObjectDirs, (op, p, e) => fileError(op, p, sp, e));
        const list = [];
    ...
    if (!url) {
        throw new Error('Could not determine the target URL: no --base-url, no baseUrl in qa-core.config.json, '
    ...
    const selected = [];
    if (opts.targets) {
        const sigOfCall = new Map();
        for (const c of calls)
            sigOfCall.set(c, argsSignature(c.method, c.args));
        ...
        for (const gotos of plan.gotosByFile.values()) {
            for (const g of gotos)
                addOrigin(resolveRoute(url, g.route));
        }
        for (const route of plan.overrideByFile.values())
            addOrigin(resolveRoute(url, route));
        const traceUrlProblem = (u) => {
            // Hard rejections always apply: about:blank, chrome-error://, and
            // anything unparseable can never be the failure page.
            if (!/^https?:\/\//.test(u))
                return `non-http page "${u}"`;
            try {
        ...
        const trustedNoted = new Set();
        const urlsByCall = new Map();
        const strictCalls = new Set();
        for (const target of opts.targets) {
            ...
            for (const frame of target.locations ?? []) {
                ...
                if (structuralInFile.length > 0) {
                    structuralInFile.sort((a, b) => Math.abs(a.line - frame.line) - Math.abs(b.line - frame.line) || a.line - b.line);
            ...
            for (const call of matches) {
                const set = urlsByCall.get(call) ?? new Set();
                if (targetUrl)
                    set.add(targetUrl);
                urlsByCall.set(call, set);
                if (target.strict)
                    strictCalls.add(call);
            }
        }
    ...
    let authMode = storageState ? 'state' : authSetup ? 'setup' : null;
    let browser;
    try {
        ...
        const resolvedOutcome = async (call, locator, level, arg, frameChain, confirmToken) => {
        ...
        const fuzzyProbe = async (call, source) => {
            let scanned;
            try {
            ...
            if (verdict.kind === 'ambiguous')
                return { kind: 'ambiguous', closeMatches: verdict.displays };
            ...
            if (ladder && !ladder.ambiguous && (ladder.frameChain?.length ?? 0) === 0) {
                ...
                if (sameEl)
                    return resolvedOutcome(call, ladder.locator, ladder.level, ladder.arg, undefined, value);
            }
        ...
        const tagFixProbe = async (call) => {
            if (call.level !== 'css' || !call.args.css)
                return null;
            const scope = scopeFor(page, call.frameChain);
            const unique = [];
            for (const corrected of tagTypoCorrections(call.args.css)) {
                ...
                if (n === 1)
                    unique.push({ css: corrected, locator });
            }
            if (unique.length !== 1)
                return null;
            const hit = unique[0];
            ...
            if (kindConflict(expectedKindsOf(call), candidateKind))
                return null;
            const newRaw = emitReplacement(call, 'css-tag-fix', hit.css, call.frameChain.length > 0 ? call.frameChain : undefined);
            return {
        ...
        const describeMatches = async (matches) => {
            const out = [];
            let n = 0;
            try {
            ...
            for (let i = 0; i < Math.min(n, 5); i++) {
                const d = await matches.nth(i).evaluate((el) => {
                    const generatedId = (v) => /^[_:]?r[_:]?(?=[0-9a-z]*\d)[0-9a-z]+[_:.-]/i.test(v)
                        || v.split(/[^A-Za-z0-9]+/).filter(Boolean)
                            .every((w) => /^\d+$/.test(w) || (/^[0-9a-f]{4,}$/i.test(w) && /\d/.test(w)));
                    if (el.id && !generatedId(el.id))
                        return '#' + el.id;
                    const name = el.getAttribute('name');
                    if (name && !generatedId(name))
                        return el.tagName.toLowerCase() + '[name="' + name + '"]';
                    const aria = el.getAttribute('aria-label');
                    if (aria)
                        return el.tagName.toLowerCase() + ' "' + aria + '"';
                    const labels = el.labels;
                    const lt = labels && labels.length > 0
                        ? (labels[0].textContent ?? '').replace(/\s+/g, ' ').trim().slice(0, 40) : '';
                    if (lt)
                        return el.tagName.toLowerCase() + ' "' + lt + '"';
        ...
        const distinguishers = async (matches, n) => {
            const infos = [];
            for (let i = 0; i < Math.min(n, 5); i++) {
                const d = await matches.nth(i).evaluate((el) => ({
                    id: el.id || '',
            ...
            for (const d of infos) {
                for (const c of new Set(d.classes))
        ...
        const probeCall = async (call, strict) => {
            ...
            if (stateGatedRole(call)) {
                ...
                if (gatedSource && call.frameChain.length === 0) {
                    ...
                    if (fz && fz.kind === 'unresolved' && fz.closest)
                        return fz;
                }
                return { kind: 'unresolved' };
            }
            ...
            if (humanized && humanized !== token)
                tokens.push(humanized);
            const stripped = stripAutoSuffixes(humanized || token);
            if (stripped && norm(stripped).length >= 2 && !tokens.includes(stripped))
        ...
        for (const route of routeOrder) {
            ...
            if (pageSpot(finalUrl) !== pageSpot(route)) {
                ...
                for (const t of tasks) {
                    if (t.routes.includes(route))
                        t.outcomes.set(route, { kind: 'redirected', to: landLabel });
                }
                continue;
            }
            opts.onEvent?.({ type: 'opened_page', url: route });
            const closedRoots = await page.evaluate(() => window.__qaCoreClosedShadowRoots ?? 0).catch(() => 0);
            if (closedRoots > 0)
                closedRootRoutes.add(route);
            for (const t of tasks) {
                if (!t.routes.includes(route))
                    continue;
        ...
        for (const t of tasks) {
            ...
            if (strictMulti && strictMulti.kind === 'strict-multi') {
                refuse(call, `locator matches ${strictMulti.count} elements identical by accessible name; `
                    ...
                    + `(candidates: ${strictMulti.candidates.join(' / ')})`, true);
                continue;
            }
            ...
            if (ambiguousEntries.length > 0) {
                ...
                const closeMatches = [...new Set(ambiguousEntries.flatMap((e) => e.o.kind === 'ambiguous' ? (e.o.closeMatches ?? []) : []))];
                const named = [...new Set(ambiguousEntries.flatMap((e) => e.o.kind === 'ambiguous' ? (e.o.candidates ?? []) : []))];
            ...
            const resolvedEntries = entries.flatMap((e) => e.o.kind === 'resolved' ? [{ route: e.route, o: e.o }] : []);
            ...
            if (resolvedEntries.length === 0) {
                ...
                const withClosedNote = (reason) => t.routes.some((r) => closedRootRoutes.has(r))
            ...
            const clash = resolvedEntries.find((e) => !e.o.roleCorrected && kindConflict(expectedKinds, e.o.candidateKind));
            if (clash) {
            ...
            const prefLevel = level === 'css-tag-fix' ? 'css' : level;
            ...
            if (unconfirmed.length > 0) {
                ...
                refuse(call, unconfirmed.some((e) => e.o.unstableMatch)
                    ...
                    : first.mismatch
                        ...
                        : 're-resolved to a different element, not healing (would be wrong)', false);
                continue;
            }
    ...
    const healPlan = [...editsByFile].map(([file, edits]) => ({
        file,
        src: files.find((f) => f.path === file).src,
        edits,
    }));
    const filesWritten = write ? applyHealPlan(healPlan) : [];
    opts.onEvent?.({
