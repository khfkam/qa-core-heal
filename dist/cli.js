...
import { applyHealPlan, applyHealPlanExcluding, collectSpecFiles, heal, selectorSignature } from './heal.js';
...
import { classifyFailure, collectTests, parseConsent, parseJsonReport, traceFailureUrl } from './run.js';
...
function parseCliArgs(argv) {
    ...
    try {
        ...
        const first = e.message.split(/\.\s/)[0].replace(/\.$/, '');
    ...
    out.configPath = v.config;
    out.baseUrl = v['base-url'];
    out.project = v.project;
    out.storageState = v['storage-state'];
    out.authSetup = v['auth-setup'];
    out.auditLog = v['audit-log'];
    out.output = v.output;
    if (v['auth-setup-timeout'] !== undefined) {
        const s = Number(v['auth-setup-timeout']);
        if (!Number.isFinite(s) || s <= 0)
            fail('--auth-setup-timeout expects a positive number of seconds.');
        out.authSetupTimeout = s * 1000;
    }
    if (v['settle-ms'] !== undefined) {
        const n = Number(v['settle-ms']);
        if (!Number.isFinite(n) || n < 0)
            fail('--settle-ms expects a non-negative number of milliseconds.');
        out.settleMs = n;
    }
    for (const r of v.route ?? []) {
        const eq = r.indexOf('=');
        if (eq <= 0)
            fail('--route expects <file>=<route>, e.g. --route pages/login-page.ts=/login');
    ...
    if (v['no-verify'] === true)
        out.verify = false;
    else if (v.verify === true)
        out.verify = true;
    out.specPath = positionals[0];
    if (positionals.length > 1)
        fail(`Unknown argument: ${positionals[1]}`);
...
function locatorVerdict(l, applied, testTitle, verified) {
    return {
        spec: l.file,
        testTitle,
...
function logChildRun(cli, run) {
    if (cli.verbose)
        console.error(`[verbose] child: ${run.command} (exit ${run.status ?? 'spawn-error'})`);
    if (cli.debug) {
        if (run.stdout)
            console.error(`[debug] child stdout:\n${run.stdout}`);
        if (run.stderr)
...
async function main() {
    ...
    const verboseNote = cli.verbose ? (line) => console.error(line) : undefined;
    let specs = [];
    if (cli.specPath) {
    ...
    const runPass = async (writePass, events, targets) => heal({
    ...
    const sayHeader = (suffix) => {
        if (specs.length === 1)
            say(`▸ Healing ${path.relative(process.cwd(), specs[0])}${suffix}`);
        else
            say(`▸ Healing ${specs.length} spec files jointly${suffix}`);
    };
    const printDiff = (proposed) => {
        if (cli.json)
            return;
    ...
    const writeAudit = (heals, verifiedFor, revertedFor = () => false) => {
        const auditEntries = heals.map((h) => ({
    ...
    const scanRevertedKeys = new Set();
    ...
    if (applied && result.healed.length > 0) {
        ...
        const verifiedBySpec = new Map();
        if (verify) {
            for (const spec of specs) {
                const gathered = result.specFiles[spec] ?? [];
                if (!gathered.some((f) => healedFiles.has(f)))
                    continue;
                const root = projectRootFor(spec);
        ...
        const revertedHeals = new Set();
        if (verify) {
            for (const h of result.healed) {
                const owners = specs.filter((sp) => (result.specFiles[sp] ?? []).includes(h.file));
                if (owners.some((sp) => verifiedBySpec.get(sp) === false))
                    revertedHeals.add(h);
            }
            if (revertedHeals.size > 0) {
                applyHealPlanExcluding(result.plan, (file, edit) => [...revertedHeals].some((h) => h.file === file && h.line === edit.line && h.new === edit.newRaw));
 ...
 */
async function runFirstFlow(ctx) {
    ...
    const titleFor = (old) => {
        ...
        for (const t of targets) {
            if (t.test && selectorSignature(t.selector) === sig)
                return t.test;
        }
        return null;
    };
    const emit = (result, applied, nonLocator, unmatchedTargets = [], rerunPassed = null) => {
        const locators = result?.locators ?? [];
        const count = (s) => locators.filter((l) => l.status === s).length;
        if (cli.json) {
            const verdicts = [
                ...locators.map((l) => {
                    ...
                    const v = locatorVerdict(l, applied, titleFor(l.old), reverted ? false : (applied && verify ? rerunPassed : null));
                    return { ...v, reverted };
                }),
                ...nonLocator.map((n) => ({
                    spec: n.file ?? '',
                    testTitle: n.test,
                    classification: 'non-locator',
                    message: n.reason,
                    healApplied: false,
                    before: null,
                    after: null,
    ...
    const fileSkipped = preview.unmatchedTargets.filter((u) => u.locations?.some((l) => erroredFiles.has(path.resolve(l.file))));
    ...
    if (applied && verify && result.healed.length > 0) {
        ...
        if (rerunPassed) {
            ...
            for (const t of locatorTests)
                verifiedByTitle.set(t.title, true);
        }
        else {
            ...
            for (const h of result.healed) {
                ...
                if (!title || verifiedByTitle.get(title) !== true)
                    revertedHeals.add(h);
            }
            if (revertedHeals.size > 0) {
                applyHealPlanExcluding(result.plan, (file, edit) => [...revertedHeals].some((h) => h.file === file && h.line === edit.line && h.new === edit.newRaw));
    ...
    if (applied && result.healed.length > 0) {
        const relOf = (h) => path.relative(process.cwd(), h.file).split(path.sep).join('/');
        writeAudit(result.healed, (h) => {
            if (!verify)
                return false;
            if (rerunPassed === true)
