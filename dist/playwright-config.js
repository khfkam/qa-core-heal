...
export function supportsTypeStripping(version = process.versions.node) {
 ...
 * flag is unnecessary.
 */
export function needsStripFlag(version = process.versions.node) {
    const [major, minor] = versionParts(version);
    if (major === 22)
        return minor >= 6 && minor < 18;
    if (major === 23)
        return minor < 6;
    return false;
}
/** The exact message the CLI exits with when a .ts config needs newer Node. */
export function typeStrippingGateMessage(version = process.versions.node) {
 ...
 */
export async function resolvePlaywrightConfig(dir, project) {
    const configPath = findPlaywrightConfig(dir);
    if (!configPath)
        return null;
    const isTs = configPath.endsWith('.ts');
    if (isTs && !supportsTypeStripping())
        return { configPath, baseUrl: null };
    const args = ['--no-warnings'];
    if (isTs && needsStripFlag())
    ...
    return new Promise((resolve) => {
        ...
        child.stdio[3].on('data', (d) => { out += d.toString(); });
        child.on('error', (e) => {
            clearTimeout(timer);
...
export async function resolvePlaywrightBaseUrl(dir) {
